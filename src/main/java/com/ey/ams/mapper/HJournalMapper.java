...
public class HJournalMapper extends BaseMapper implements RowMapper<HJournal> {

    @Override
    public HJournal mapRow(ResultSet rs, int rowNum) throws SQLException {
	HJournal ob = new HJournal();
	int colindex = 0;
	...
	colindex = findColumn(rs, "drkey");
	if (colindex > 0) {
	    ob.setDrkey(rs.getString(colindex));
	}
	
	colindex = findColumn(rs, "crkey");
	if (colindex > 0) {
	    ob.setCrkey(rs.getString(colindex));
	}
	...
	colindex = findColumn(rs, "drctaxamount");
	if (colindex > 0) {
	    ob.setDrctaxamount(rs.getDouble(colindex));
	}
	
	colindex = findColumn(rs, "crctaxamount");
	if (colindex > 0) {
	    ob.setCrctaxamount(rs.getDouble(colindex));
	}
	...
	colindex = findColumn(rs, "receivecode");
	if (colindex > 0) {
	    ob.setReceivecode(rs.getString(colindex));
	}
